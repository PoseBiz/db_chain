  ...
  def get_pg_extras!
    ...
    databases.each do |name,attrs|
      ...
      threads << Thread.new do
        ...
        attrs[:running_queries_count] = conn.exec("SELECT COUNT(*) AS count FROM pg_stat_activity  WHERE usename != 'collectd' AND state='active' AND query NOT LIKE '<%'")[0]["count"].to_i

        attrs[:slow_queries] = conn.exec("SELECT query FROM pg_stat_activity  WHERE usename != 'collectd' AND query NOT LIKE '<%' AND state='active' AND (now()-query_start) > '20 seconds' ORDER BY query_start ASC").map {|r| r["query"]}
